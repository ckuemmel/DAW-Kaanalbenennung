...
def find_header_row(ws) -> tuple[int, dict[str, int]]:
    """Sucht die Header-Zeile und gibt Zeilennummer und Spalten-Mapping zurück"""
    for row in range(1, min(ws.max_row + 1, 20)):  # Erste 20 Zeilen durchsuchen
        columns = {}
        found_any = False
        for col in range(1, ws.max_column + 1):
            ...
            if value:
                ...
                if "name" in value or "instrument" in value:
                    columns["name"] = col
                    found_any = True
                elif "kanal" in value or "channel" in value:
                    columns["channel"] = col
                    found_any = True
                elif "mikrofon" in value or "mic" in value:
                    columns["mic"] = col
                    found_any = True
        if found_any:
            return row, columns
    return 1, {}  # Fallback: erste Zeile
...
def read_names_from_xlsx(path: Path, args) -> list[tuple[str, str | None, str | None]]:
    ...
    # Header-Zeile und Spalten finden
    header_row, columns = find_header_row(ws)
    
    if "name" not in columns:
        print("Fehler: Keine Namensspalte gefunden ('Name' oder 'Instrument')")
        print("\nVerfügbare Spalten:")
        for col in range(1, ws.max_column + 1):
    ...
    # Namen einlesen (ab der nächsten Zeile nach dem Header)
    for row in range(header_row + 1, ws.max_row + 1):
        name = ws.cell(row=row, column=columns["name"]).value
        if name:
            name = str(name).strip()
            if not name:
                continue
            ...
            if "channel" in columns:
                channel = ws.cell(row=row, column=columns["channel"]).value
                if channel:
                    channel = str(channel).strip()
            
            if "mic" in columns:
                mic = ws.cell(row=row, column=columns["mic"]).value
                if mic:
                    mic = str(mic).strip()
                    
            names.append((name, channel, mic))
    
    print(f"INFO: {len(names)} Namen gefunden")
    if names:
        print("Beispiel:", names[0])
