...
def read_names_from_xlsx(path: Path, args) -> list[tuple[str, str | None, str | None]]:
    """Liest Namen aus einer Excel-Datei mit festen Spalten:
    A: Instrumente
    B: Kanäle
    E: Mikrofone
    """
    ...
    # Feste Spaltenzuordnung (0-basiert)
    NAME_COL = args.name_spalte      # Standard: Spalte A (1)
    CHANNEL_COL = args.kanal_spalte  # Standard: Spalte B (2)
    MIC_COL = args.mic_spalte        # Standard: Spalte E (5)
    
    print(f"\nDEBUG: Spalten-Konfiguration:")
    print(f"  Namen: Spalte {NAME_COL}")
    print(f"  Kanäle: Spalte {CHANNEL_COL}")
    print(f"  Mikrofone: Spalte {MIC_COL}")
    
    # Zeige erste Zeile zur Kontrolle
    print("\nDEBUG: Erste Zeile (Header):")
    for col in [NAME_COL, CHANNEL_COL, MIC_COL]:
        val = read_cell(ws, 1, col)
        print(f"  Spalte {col}: {val}")
    
    # Namen ab Zeile 2 einlesen (Zeile 1 ist Header)
    print("\nDEBUG: Lese Namen...")
    start_row = args.start_zeile if args.start_zeile else 2
    for row in range(start_row, ws.max_row + 1):
        name = read_cell(ws, row, NAME_COL)
        if name:
            channel = read_cell(ws, row, CHANNEL_COL) if args.use_kanal else None
            mic = read_cell(ws, row, MIC_COL) if args.use_mic else None
            names.append((name, channel, mic))
            if args.debug and len(names) <= 3:
                print(f"\nDEBUG: Zeile {row}:")
                print(f"  Name ({NAME_COL}): {name}")
                print(f"  Kanal ({CHANNEL_COL}): {channel}")
                print(f"  Mikrofon ({MIC_COL}): {mic}")
