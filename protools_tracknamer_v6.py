...
def read_names_from_xlsx(path: Path, args) -> list[tuple[str, str | None, str | None]]:
    ...
    for row in range(2, ws.max_row + 1):
        ...
        if name:
            channel = read_cell(ws, row, CHANNEL_COL) if args.kanal_spalte else None
            mic = read_cell(ws, row, MIC_COL) if args.mikrofon_spalte else None
...
def main():
    ...
    parser.add_argument("--kanal-spalte", action="store_true", help="Kanalnummer zum Namen hinzufügen (Spalte B)")
    parser.add_argument("--mikrofon-spalte", action="store_true", help="Mikrofonname zum Namen hinzufügen (Spalte E)")
