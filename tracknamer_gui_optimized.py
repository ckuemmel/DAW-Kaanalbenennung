...
class TrackNamerGUI(tk.Tk):
    def create_widgets(self):
        ...
        self.table_frame = tk.Frame(self, bg="lightgray", height=400)
        self.table_frame.pack(expand=True, fill="both", padx=10, pady=5)
        
        # Treeview mit Mehrfachauswahl - zunächst ohne columns definiert
        self.tree = ttk.Treeview(self.table_frame, show="tree headings", selectmode="extended", height=15)
        ...
        # Grid-Konfiguration für automatische Größenanpassung
        self.table_frame.grid_rowconfigure(0, weight=1)
        self.table_frame.grid_columnconfigure(0, weight=1)
        
        # Frame muss seine Größe beibehalten
        self.table_frame.pack_propagate(False)
        
        # Debug-Label für Sichtbarkeit
        debug_label = tk.Label(self.table_frame, text="TreeView sollte hier erscheinen", bg="yellow")
        debug_label.grid(row=2, column=0, columnspan=2, sticky="ew")
    ...
    def load_excel(self):
        ...
        try:
            ...
            print(f"Erste paar Datenzeilen: {self.data[:3] if self.data else 'Keine Daten'}")
    ...
    def show_table(self):
        print(f"show_table() aufgerufen - Datenlänge: {len(self.data) if hasattr(self, 'data') else 'Keine data'}")
        ...
        # Wichtig: Spalten EXPLIZIT setzen
        self.tree.configure(columns=columns)
        self.tree.configure(show="tree headings")  # Sowohl Tree als auch Headings anzeigen
        
        print(f"Spalten gesetzt: {columns}")
        
        # Spaltenüberschriften und -breiten setzen
        for i, col in enumerate(columns):
            col_id = f"#{i+1}" if i > 0 else "#0"  # #0 für erste Spalte, #1, #2, etc. für weitere
            ...
            if col == "Export":
                ...
                self.tree.column(col_id, width=200, anchor='w', minwidth=120)
                
            print(f"Spalte {i}: {col} -> {col_id}")
        
        print(f"TreeView konfiguriert mit {len(columns)} Spalten")
        ...
        for idx, row in enumerate(self.data):
            try:
                ...
                has_instrument = len(row) > 1 and row[1] and str(row[1]).strip() != ''
                ...
                # Erste Spalte (Export) als text, Rest als values
                self.tree.insert('', 'end', iid=str(idx), text=checkbox_text, values=row_data)
                if idx < 5:  # Nur erste 5 Zeilen loggen
                    print(f"Zeile {idx} eingefügt: text='{checkbox_text}', values={row_data}")
                
            except Exception as e:
                print(f"Fehler beim Einfügen von Zeile {idx}: {e}, Row: {row}")
                continue
        
        print(f"Tabelle fertig: {len(self.data)} Zeilen eingefügt")
        self.tree.update()
        
        # Info über die Auswahl aktualisieren
        self.update_selection_info()
