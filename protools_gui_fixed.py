...
class ProToolsGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Pro Tools Track Namer v1.0")
    ...
    def create_widgets(self):
        ...
        instructions = ("1. Excel-Datei mit Instrumentenliste auswählen\n"
                       ...
                       "4. 'Spuren benennen' klicken für automatische F9-Sequenz\n"
    ...
    def rename_tracks(self):
        """Benennt Spuren in Pro Tools durch F9-Tastendruck"""
        if self.running:
            messagebox.showinfo("Info", "Es läuft bereits eine Aktion. Bitte warten oder ESC drücken.")
            return
        ...
        def run_renaming():
            try:
                ...
                # GUI-Update im Main Thread
                self.root.after(0, lambda: self.status_var.set(f"Benenne {len(names)} Spuren..."))
                self.root.after(0, lambda: self.create_button.configure(state="disabled"))
                self.root.after(0, lambda: self.rename_button.configure(state="disabled"))
                ...
                self.root.after(0, lambda: messagebox.showinfo("Spurbenennung", 
                    ...
                    f"Stelle sicher, dass:\n"
                    f"• Pro Tools geöffnet und aktiv ist\n" 
                    f"• Die erste Spur ausgewählt ist\n"
                    f"• Das Mix-Fenster im Vordergrund ist\n\n"
                    f"Drücken Sie OK und die F9-Sequenz startet..."))
                
                # F9-Sequenz ausführen
                import time
                time.sleep(2)  # Kurze Pause nach Dialog
                
                self.tracknamer.name_tracks_with_f9(
                    lambda msg: self.root.after(0, lambda: self.task_completed("Alle Spuren wurden erfolgreich benannt!")))
...
def main():
    """Hauptfunktion"""
    print("Pro Tools Track Namer v1.0")
    print("==========================")
